...
function runTests(array $tests)
{
    ...
    foreach ($tests as $testData) {
        $map = new Map();
        $map->processSteps($testData['wire1']);
        $map->processSteps($testData['wire2']);

        $mapReader = new MapReader($map);
        $distance = $mapReader->getShortestIntersectionDistance();

        if ($testData['shortest'] !== $distance) {
            $errors[] = [
                'testData' => $testData,
                'actual' => $distance,
            ];
...
class Map
{
    ...
    protected const DOWN = 'D';
    protected const UP = 'U';
    ...
    public function processSteps(string $steps): self
    {
        $this->wireId++;

        $movements = static::parseMovements($steps);

        $currentStep = [self::X => 0, self::Y => 0, 'distance' => 0];
        foreach ($movements as $movement) {
            $currentStep = $this->processMovement($currentStep, $movement);
        }
    ...
    protected function processMovement(array $currentStep, string $movement): array
    {
        ['direction' => $direction, 'step' => $step] = static::parseMovement($movement);

        $method = static::getStepMethod($direction);

        return $this->$method($currentStep, $step);
    }

    protected static function parseMovements(string $movements): array
    {
        return explode(',', $movements);
    }

    protected static function parseMovement(string $movement): array
    {
        return [
            'direction' => $movement[0],
            'step' => (int) substr($movement, 1),
        ];
    }

    protected function rightMovement(array $currentStep, $steps): array
    {
        $from = $currentStep[self::X] + 1;
        $to = $currentStep[self::X] + $steps;

        for ($x = $from; $x <= $to; ++$x) {
            $currentStep[self::X] = $x;

            $currentStep = $this->markVisited($currentStep);
        }

        return $currentStep;
    }

    protected function leftMovement(array $currentStep, $steps): array
    {
        $from = $currentStep[self::X] - 1;
        $to = $currentStep[self::X] - $steps;
    ...
    protected function upMovement(array $currentStep, $steps): array
    {
        $from = $currentStep[self::Y] + 1;
        $to = $currentStep[self::Y] + $steps;

        for ($y = $from; $y <= $to; ++$y) {
            $currentStep[self::Y] = $y;
    ...
    protected function downMovement(array $currentStep, $steps): array
    {
        $from = $currentStep[self::Y] - 1;
        $to = $currentStep[self::Y] - $steps;
    ...
    protected static function getStepMethod(string $direction): string
    {
        switch ($direction) {
            case self::RIGHT:
            ...
            default:
                throw new \Exception('Operation not found: ' . $direction);

                break;
        }
    }

    protected function markVisited(array $currentStep): array
    {
        $x = $currentStep[self::X];
        $y = $currentStep[self::Y];

        if (!isset($this->map[$x][$y])) {
            $this->map[$x][$y] = [];
        }

        if (!array_key_exists($this->wireId, $this->map[$x][$y])) {
            $this->map[$x][$y][$this->wireId] = ['wireId' => $this->wireId, 'distance' => $currentStep['distance']];
        }
...
class MapReader
{
    ...
    protected function calculateIntersections(): void
    {
        foreach ($this->map as $x => $row) {
            foreach ($row as $y => $column) {
                if (
                    ...
                    $wireDistance = 0;
                    foreach ($column as $wireId => $wireData) {
                        $wireDistance += $wireData['distance'];
                    }
    ...
    public function getClosestIntersectionDistance(): int
    {
        ...
        foreach ($this->intersections as $intersection) {
            $distances[] = abs($intersection['x']) + abs($intersection['y']);
        }
    ...
    public function getShortestIntersectionDistance(): int
    {
        $this->calculateIntersections();
        ...
        foreach ($this->intersections as $intersection) {
            $distances[] = $intersection['wireDistance'] + 2;
        }
...
$map = new Map();
$map->processSteps($input1);
$map->processSteps($input2);
